...
[1, -1, 0, -1, -1, -1, -1, -1, -1]
*/
var board_state = [-10,-10,-10,-10,-10,-10,-10,-10,-10]
...
function winner_check(array){

	if(array[0] + array[1] + array[2] == 3 || array[3] + array[4] + array[5] == 3
		 || array[6] + array[7] + array[8] == 3 || array[0] + array[3] + array[6] == 3
		 || array[1] + array[4] + array[7] == 3 || array[2] + array[5] + array[8] == 3
		 || array[0] + array[4] + array[8] == 3 || array[2] + array[4] + array[6] == 3)
		 {
			 alert("Winner is X");
			reset_play();

	}
	if(array[0] + array[1] + array[2] == 0 || array[3] + array[4] + array[5] == 0
		 	|| array[6] + array[7] + array[8] == 0|| array[0] + array[3] + array[6] == 0
			|| array[1] + array[4] + array[7] == 0 || array[2] + array[5] + array[8] == 0
			|| array[0] + array[4] + array[8] == 0 || array[2] + array[4] + array[6] == 0){
			alert("Winner is O");
			reset_play();
...
function begin_play(){
	...
	if(started){
		alert("Game has already started. Press reset in order to start again");
	...
	if(player1.value.length == 0 || player2.value.length== 0){
		alert("Both name fields must be filled in order to start");
	}else{
		started = true;
		player1.value +=  " (X)";
		player2.value +=  " (O)";
...
function reset_play(){
	...
	board_state = [-10,-10,-10,-10,-10,-10,-10,-10,-10];
	for( i = 0; i < 9; i++){
			...
			var place = document.getElementById(input.value);
				if(turn == 1){
					if(place.innerHTML == "X" || place.innerHTML == "O"){
						alert("Somone has already made a move in that location");
					}else{
					place.innerHTML = "X";
					turn = 0;
					currentTurn = " O";
					winner_check(board_state);
					document.getElementById('turn_info').innerHTML =  "Turn for:"+ currentTurn.bold();
				...
				alert("That location does not exist on the board. Please enter a valid location");
			}
			document.getElementById('move_text_id').value = '';
}else{
	alert("Game has not started yet. Push begin play");
}
}
